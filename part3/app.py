...
def registerAuth():
    ...
    if (user_type == 'custo'):
        ...
        cursor.execute(inst, (username, password, name, adrr_building_num, adrr_street, addr_city, addr_state, phone_num, passport_num, passport_exp, passport_country, date_of_birth))
...
def searchPublicView():
    ...
    query =  'SELECT distinct f.airline_name, f.flight_num, de.airport_name as depart_airport, depart_date_time, ar.airport_name as arrive_airport, arrive_date_time, f.status'
                 ...
                 'WHERE ((f.depart_airport_code = de.code AND f.arrive_airport_code = ar.code) AND  ("{}" = de.city or "{}" = de.airport_name)'.format(dept, dept) ' AND "{}" = ar.city or "{}" = ar.airport_name)'.format(arri, arri)
                 'AND ("{}" = date(f.depart_date_time))'.format(dept_date)
                 'ORDER BY f.depart_date_time)
    if "return_date" in request.form:
        return_date = request.form['return_date']
        query += 'OR ((f.depart_airport_code = de.code AND f.arrive_airport_code = ar.code) AND ("{}" = de.city or "{}" = de.airport_name)'.format(dept, dept) ' AND "{}" = ar.city or "{}" = ar.airport_name)'.format(arri, arri)
                 'AND ("{}" = date(f.depart_date_time))'.format(return_date)
                 'ORDER BY f.depart_date_time)
...
def history():
    ...
    return render_template('custo_history.html', total_spent=total_spent, past_six=past_six, flights_history = flights_history)
