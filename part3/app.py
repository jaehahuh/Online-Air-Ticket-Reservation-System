...
def StaffCreate():
	if confirmstaff():
		...
		return redirect('/StaffHome') #After adding you can either logout or go hom
...
def AddAirplane():
	if confirmstaff():
		...
		conn.commit() # python does not auto commit, therefore need to send commit after making changes to data
		cursor.close()
		return redirect('/logout')
	else:
		return redirect('/StaffHome') #After adding you can either logout or go home to perform more actions.
...
@app.route('/StaffFlightLayout', methods = ['GET', 'POST'])
def StaffFlightLayout():
	if confirmstaff():
		query = "SELECT status, airline_name, flight_num, depart_date_time, " \
		"arrive_date_time, depart_airport_code, arrive_airport_code FROM flight, " \
...
def StaffChangeFlights():
	if confirmstaff():
		...
		if data:
			query = "UPDATE flight SET flight.flight_status = %s WHERE airline_name = %s AND flight_num = %s"
...
def ViewTopDestinations():
	if confirmstaff():
		cursor = conn.cursor()
		month = "SELECT arrival_airport_code, count(t.ticket_id) FROM flight f JOIN ticket t ON f.flight_num = t.flight_num " \
